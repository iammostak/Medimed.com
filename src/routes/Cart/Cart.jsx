import { Box, Button, Flex, Heading, Skeleton, Text, useToast } from "@chakra-ui/react";
 ...
 setCartState({...cartState,loading:true,success:false,error:false})
    getCartData()
      .then((res) => {
        setCartData([...res.data])
        // console.log(res.data);
        setCartState({...cartState,loading:false,success:true,error:false})
      })
      .catch((e) => {
        console.log(e);
...
<Link to="/"  ><Button bg={mainColor} color="white" colorScheme={"none"} >Back To Home page</Button></Link>
    ...
    <Box 
      ...
      marginTop={10}
    >
      {cartData&&<PaymentDetails price={fullMrp} total={finalePrice} cartState={cartState}   />}
